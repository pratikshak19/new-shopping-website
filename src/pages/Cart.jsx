...
import { smartCartSuggestion } from '../lib/styleEngine'
import { useStore } from '../context/StoreContext'
import ProductImage from '../components/ProductImage'
import { Icon } from '../components/Icons'

export default function Cart() {
  const { cartDetailed, totals, updateQty, removeFromCart, applyCoupon, coupon, clearCoupon, giftWrap, setGiftWrap, coupons, visibleProducts, addToCart, budgetLock, user } = useStore()
  const smart = smartCartSuggestion(
    cartDetailed.map((l) => l.product),
    visibleProducts,
    budgetLock || 0
  )
  ...
  return (
    <div className="wrap">
      ...
      <div className="cart-layout">
        <div>
          {smart && (
            <aside className="smart-cart">
              <p>🛍️ {smart.text}</p>
              {smart.product && (
                <button className="btn btn-ghost" type="button" onClick={() => addToCart(smart.product.id)}>
                  Add {smart.product.name} · {formatINR(smart.product.price)}
                </button>
              )}
            </aside>
          )}
        ...
        <aside className="summary">
          ...
          <Link className="btn btn-primary btn-block" to={user ? '/checkout' : '/login?next=/checkout'}>
