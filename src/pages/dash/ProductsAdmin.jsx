...
export default function ProductsAdmin({ role }) {
  ...
  const boss = user?.role === 'admin' || role === 'admin'
  const list = products.filter((p) => p.status !== 'hidden')
  ...
  return (
    <div>
      ...
      <p className="muted">
        {boss ? (
          <>
            Only you can change the catalogue. Guided add:{' '}
            <a href={`/${role}/add`}>Add new item</a>.
          </>
        ) : (
          'Catalogue is view-only. Ask the store admin to add or remove items.'
        )}
      </p>
      {boss && <form className="dash-form" onSubmit={submit}>
        <div className="field">
          <label>Name</label>
